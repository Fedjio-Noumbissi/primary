...
router.post('/evaluations', async (req, res) => {
  try {
    ...
    const now = new Date()
    const [result] = await pool.query(
      'INSERT INTO Evaluation (note, appreciation, matricule, idEpreuve, idCours, idSession, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [note || 0, appreciation || '', matricule, idEpreuve, idCours, idSession, now, now]
    )
...
router.post('/evaluations/bulk', async (req, res) => {
  try {
    ...
    const now = new Date()
    const values = items.map(i => [i.note || 0, i.appreciation || '', i.matricule, i.idEpreuve, i.idCours, i.idSession, now, now])
    await pool.query(
      'INSERT INTO Evaluation (note, appreciation, matricule, idEpreuve, idCours, idSession, createdAt, updatedAt) VALUES ?',
      [values]
    )
